#include "mapping/mapping.h" // 假设头文件在 include/mapping/ 下
...
// updateMap 函数逻辑主要是数学运算，几乎不需要修改，除了日志输出
...
void OccGridMap::occ2pc(sensor_msgs::msg::PointCloud2& msg, double floor, double ceil) {
  pcl::PointXYZ pt;
  pcl::PointCloud<pcl::PointXYZ> pcd;
  for (int x = 0; x < size_x; ++x) {
    for (int y = 0; y < size_y; ++y) {
...
void OccGridMap::inflate_once() {
  ...
  for (const auto& id : v0) {
    int x0 = std::max(id.x() - 1, offset_x);
    int y0 = std::max(id.y() - 1, offset_y);
    int z0 = std::max(id.z() - 1, offset_z);
    int x1 = std::min(id.x() + 1, offset_x + size_x - 1);
    int y1 = std::min(id.y() + 1, offset_y + size_y - 1);
    int z1 = std::min(id.z() + 1, offset_z + size_z - 1);
