...
#include <decomp_ros_msgs/msg/ellipsoid.hpp>
...
nav_msgs::msg::Path vec_to_path(const vec_Vecf<Dim> &vs) {
  ...
  // ROS 2 中推荐设置 header.frame_id 和 stamp，虽然这里为了保持逻辑一致没有强制加
...
decomp_ros_msgs::msg::EllipsoidArray ellipsoid_array_to_ros(const vec_E<Ellipsoid<Dim>>& Es) {
  ...
  for (unsigned int i = 0; i < Es.size(); i++) {
    ...
    for (int x = 0; x < 3; x++) {
      for (int y = 0; y < 3; y++) {
        if(x < Dim && y < Dim)
          ellipsoid.e[3 * x + y] = C(x, y); // 注意: 这里的 .E 在 ROS1->ROS2 转换中通常字段名不变，但请检查 .msg 定义是否为小写 e 或大写 E
        else
          ellipsoid.e[3 * x + y] = 0;
      }
    }
