...
#include <rviz_common/logging.hpp> // 用于 logging
#include <pluginlib/class_list_macros.hpp> // 插件宏

namespace decomp_rviz_plugins {

PolyhedronArrayDisplay::PolyhedronArrayDisplay() {
  // 属性命名空间 rviz_common::properties
...
// 参数改为 ConstSharedPtr (或者值传递 ConstSharedPtr msg)
void PolyhedronArrayDisplay::processMessage(const decomp_ros_msgs::msg::PolyhedronArray::ConstSharedPtr msg) {
  // getTransform 需要 rclcpp::Time
  if (!context_->getFrameManager()->getTransform(
          msg->header.frame_id, rclcpp::Time(msg->header.stamp), position_, orientation_)) {
    RCLCPP_DEBUG(rclcpp::get_logger("PolyhedronArrayDisplay"), 
